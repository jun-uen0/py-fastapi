...
@router.get('/api/csrf-token', response_model=Csrf)
...
@router.post('/api/user', response_model=SuccessMsg)
async def get_user_refresh_jwt(request: Request, response: Response):
  new_token, subject = auth.verity_update_jwt(request)
  response.set_cookie(
    key='access_token', value=f'Bearer {new_token}', httponly=True, samesite='none', secure=True)
  return {'message': 'Successfully refresh jwt', 'email': subject}
